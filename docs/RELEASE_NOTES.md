# Riffdown 0.2.0 preview

Folio is now **Riffdown**. This release also includes custom tint controls and one-file-at-a-time viewing with Close Document. Existing appearance settings and editor selection are retained.
...
- **Mac (Apple Silicon + Intel, macOS 12+):** `Riffdown-macOS-universal.zip`. Unzip and move Riffdown to Applications.
...
Mac Quick Look is included and experimental. Enable the extension in System Settings after opening Riffdown once. Its native renderer passes a smoke test; Finder integration needs broader testing. Quick Look uses the default style and image placeholders.
