...
WAYLAND_DEBUG=client riffdown "$FOLIO_ROOT/examples/Reading sample.md" > "$FOLIO_LOG" 2>&1 &
FOLIO_APP_PID=$!
for attempt in {1..60}; do
  if ! kill -0 "$FOLIO_APP_PID" 2>/dev/null; then cat "$FOLIO_LOG"; exit 1; fi
  if grep -q 'set_title("Riffdown")' "$FOLIO_LOG" && grep -Eq 'wl_surface@[0-9]+\.attach\(wl_buffer@' "$FOLIO_LOG"; then
    sleep 3
    kill -0 "$FOLIO_APP_PID"
    echo 'Arch Wayland smoke passed: Riffdown opened a native window and submitted a rendered buffer.'
    exit 0
  fi
  sleep 1
done
cat "$FOLIO_LOG"
echo 'Riffdown did not draw a Wayland window in time.' >&2
exit 1
