...
const SELF_ERROR: &str = "SlayDown is a viewer. Choose a different application for editing.";
...
pub async fn choose_editor(app: AppHandle) -> Result<Option<EditorInfo>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let current_executable = std::env::current_exe()
            .map_err(|error| format!("SlayDown could not check the selected application: {error}"))?;
...
fn settings_path(app: &AppHandle) -> Result<PathBuf, String> {
    app.path()
        ...
        .map_err(|error| format!("SlayDown's preferences folder could not be located: {error}"))
}
...
mod tests {
    ...
    fn executable_validation_rejects_plain_files_and_self_symlinks() {
        ...
        let current = std::env::current_exe().unwrap();
        let alias = folder.path().join("Renamed SlayDown");
