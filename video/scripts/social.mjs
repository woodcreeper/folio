...
const output = path.resolve(root, '../docs/images/folio-social.png');
...
const serveUrl = await bundle({entryPoint:path.join(root,'src/FolioSocial.tsx'), publicDir:path.join(root,'public'), outDir:path.join(root,'.cache/social')});
const composition = await selectComposition({serveUrl, id:'FolioSocial', browserExecutable, chromiumOptions});
