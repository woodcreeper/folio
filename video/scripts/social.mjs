...
const output = path.resolve(root, '../docs/images/riffdown-social.png');
...
const serveUrl = await bundle({entryPoint:path.join(root,'src/RiffdownSocial.tsx'), publicDir:path.join(root,'public'), outDir:path.join(root,'.cache/social')});
const composition = await selectComposition({serveUrl, id:'RiffdownSocial', browserExecutable, chromiumOptions});
